...
const AddMasterWalletScreen = () => {
    ...
    useEffect(()=>{
        createMaster();
        generateNewMnemonic().then(newMnemonic=>setMnemonic(newMnemonic.toString()));
    },[]);

    const createMasterWallet = ()=>{
        const {privateKey,publicKey,chainCode} = createMasterNode(mnemonic,password);
        addMaster(chainCode.toString('hex'),publicKey.toString('hex'),privateKey.toString('hex'))
        navigation.navigate("Main");
    }

    return (<View>
        <Text>{mnemonic.toString()}</Text>
        <TextInput placeholder={"optional: put PassWord!"} value={password} onChangeText={setPassword}/>
